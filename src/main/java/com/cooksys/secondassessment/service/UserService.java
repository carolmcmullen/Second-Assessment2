...
public class UserService {
	 ...
	 * the user whose username is given in the url. If there is already a
	 * following relationship between the two users, no such followable user
	 * exists (deleted or never created), or the credentials provided do not
	 * match an active user in the database, an error should be sent as a
	 * response. If successful, no data is sent.
	 */
	...
	/*
	 * Unsubscribes the user whose credentials are provided by the request body
	 * from the user whose username is given in the url. If there is no
	 * preexisting following relationship between the two users, no such
	 * followable user exists (deleted or never created), or the credentials
	 * provided do not match an active user in the database, an error should be
	 * sent as a response. If successful, no data is sent.
	 */
	...
	public List<UserWithIdDto> getFollowers(String username) throws Exception {
		User user = jpaUserRepository.findByUsername(username);
		if (user != null) {
			return user.getFollowers().stream().filter(u -> u.getIsActive().equals(true))
					.map(userMapper::toUserWithIdDto).collect(Collectors.toList());
		}
	...
	public List<UserWithIdDto> getFollowing(String username) throws Exception {
		User user = jpaUserRepository.findByUsername(username);
		if (user != null) {
			return user.getFollowing().stream().filter(u -> u.getIsActive().equals(true))
					.map(userMapper::toUserWithIdDto).collect(Collectors.toList());
		}
		throw new Exception("Cannot find user");
	}
